...
/*	from OpenSolaris "main.cc	1.158	06/12/12"	*/
	...
	if(POSIX||!strcmp(argv_zero_string, NOCATGETS("/usr/xpg4/bin/make"))) {
		...
		if(!strcmp(prognameptr, NOCATGETS("svr4.make"))) {
			...
			posix = false;
		       ...
		       if (!strcmp(prognameptr, NOCATGETS("make_xpg4"))) {
			svr4 = false;
			posix = true;
		}
	}
 ...
 */
	...
	if (prop2 == NULL) {
		...
		if ((dmake_value2 == NULL) ||
			...
			warning("Unsupported value `%s' for DMAKE_OUTPUT_MODE after -x flag (ignored)",
	...
	if (prop2 == NULL) {
		...
		if ((dmake_value2 == NULL) ||
			...
			fatal("Unknown dmake mode argument `%s' after -m flag", dmake_value2->string_mb);
		}
	}
		...
		if ((((prop = get_prop(dmake_name->prop, macro_prop)) == NULL) ||
			...
			if (empty_dmakerc) {
				if (getenv(NOCATGETS("DMAKE_DEF_PRINTED")) == NULL) {
					...
					fprintf(stdout, "dmake: defaulting to parallel mode.\n");
					fprintf(stdout, "See the man page dmake(1) for more information on setting up the .dmakerc file.\n");
				}
				dmake_mode_type = parallel_mode;
				no_parallel = false;
			}
...
#ifdef NSE
		setenv(NOCATGETS("NSE_DEP"), get_current_path());
#endif
	} else {
		maybe_append_prop(sunpro_dependencies, macro_prop)->
	...
	trace_reader = false;
 	...
 	if ((slash_ptr = strrchr(make_state->string_mb, (int) slash_char)) == NULL) {
 		...
 		*slash_ptr = (int) nul_char;
 		strcpy(make_state_dir, make_state->string_mb);
...
/*
			fatal("couldn't tell rxm to exit");
 */
			kill(rxmPid, SIGTERM);
		}
...
#else
#	define SVR4_CMD_OPTS   "-ef:ij:knpqrstV"
#endif
		...
		if (ch == '?') {
			...
			if (svr4) {
#ifdef TEAMWARE_MAKE_CMN
				fprintf(stderr,
					"Usage : %s [ -f makefile ][ -c dmake_rcfile ][ -g dmake_group ]\n", progname);
				fprintf(stderr,
					"              [ -j dmake_max_jobs ][ -m dmake_mode ][ -o dmake_odir ]...\n");
				fprintf(stderr,
					"              [ -e ][ -i ][ -k ][ -n ][ -p ][ -q ][ -r ][ -s ][ -t ][ -v ]\n");
#else
				fprintf(stderr,
					"Usage : %s [ -f makefile ]... [ -e ][ -i ][ -k ][ -n ][ -p ][ -q ][ -r ]\n", progname);
				fprintf(stderr,
					"             [ -s ][ -t ]\n");
#endif
				...
				fprintf(stderr,
					"Usage : %s [ -f makefile ][ -c dmake_rcfile ][ -g dmake_group ]\n", progname);
				fprintf(stderr,
					"              [ -j dmake_max_jobs ][ -K statefile ][ -m dmake_mode ][ -x MODE_NAME=VALUE ][ -o dmake_odir ]...\n");
				fprintf(stderr,
					"              [ -d ][ -dd ][ -D ][ -DD ][ -e ][ -i ][ -k ][ -n ][ -p ][ -P ][ -u ][ -w ]\n");
				fprintf(stderr,
					"              [ -q ][ -r ][ -s ][ -S ][ -t ][ -v ][ -V ][ target... ][ macro=value... ][ \"macro +=value\"... ]\n");
#else
				fprintf(stderr,
					"Usage : %s [ -f makefile ][ -K statefile ]... [ -d ][ -dd ][ -D ][ -DD ]\n", progname);
				fprintf(stderr,
			...
			if (!tptr) {
				...
				fatal("Missing argument after `-%c'", optopt);
			}
		    } else {	// !sun_style
			tptr = svr4 ? strchr(SVR4_CMD_OPTS, optopt) :
				strchr(SUNPRO_CMD_OPTS, optopt);
		    	if (!tptr) {
...
*/
		...
		if (current_optind < optind) {
			...
			case 2:	/* -c seen */
				argv[i] = NOCATGETS("-c");
#ifndef TEAMWARE_MAKE_CMN
				warning("Ignoring DistributedMake -c option");
#endif
				break;
			case 4:	/* -g seen */
				argv[i] = NOCATGETS("-g");
#ifndef TEAMWARE_MAKE_CMN
				warning("Ignoring DistributedMake -g option");
#endif
				break;
			case 8:	/* -j seen */
#ifdef TEAMWARE_MAKE_CMN
			...
			case 16: /* -M seen */
				argv[i] = NOCATGETS("-M");
#ifndef TEAMWARE_MAKE_CMN
				warning("Ignoring ParallelMake -M option");
#endif
				break;
			case 32: /* -m seen */
				argv[i] = NOCATGETS("-m");
#ifndef TEAMWARE_MAKE_CMN
				warning("Ignoring DistributedMake -m option");
#endif
				break;
#ifndef PARALLEL
			case 128: /* -O seen */
			...
			case 512:	/* -o seen */
				argv[i] = NOCATGETS("-o");
#ifndef TEAMWARE_MAKE_CMN
				warning("Ignoring DistributedMake -o option");
#endif
				break;
			case 1024: /* -x seen */
				argv[i] = NOCATGETS("-x");
#ifndef TEAMWARE_MAKE_CMN
				warning("Ignoring DistributedMake -x option");
#endif
				break;
...
parse_command_option(register char ch)
{
...
#else
		warning("Ignoring ParallelMake -R option");
#endif
		return 0;
	...
	case 'v':			/* Version flag */
		if (invert_this) {
			...
			exit(0);
#else
			warning("Ignoring DistributedMake -v option");
#endif
		}
		return 0;
...
int   done=0;
	...
	if ((sccs_dir_path != NULL) &&
		...
		if ((pwent = getpwnam(sccs_dir_path)) == NULL) {
		  ...
		  sprintf(path, NOCATGETS("%s/src"), pwent->pw_dir);
		  if (access(path, F_OK) == 0) {
			sccs_dir_path = path;
			done = 1;
		  } else {
			sprintf(path, NOCATGETS("%s/source"), pwent->pw_dir);
			if (access(path, F_OK) == 0) {
				sccs_dir_path = path;
				done = 1;
			}
		...
		if (!done) {
		    if (getcwd(cwdpath, MAXPATHLEN - 1 )) {
		       ...
		       sprintf(path, NOCATGETS("%s/%s"), cwdpath,sccs_dir_path);
		       if (access(path, F_OK) == 0) {
		        	sccs_dir_path = path;
				done = 1;
		        } else {
 ...
 */
	...
	if (!ignore_default_mk) {
		...
		read_makefile(default_makefile,
				     true,
				     false,
				     true);
	}
 ...
 */
	...
	for (i = 1; i < argc; i++) {
		if (argv[i] &&
			...
			read_makefile(primary_makefile, true, true, true);
 ...
 */
	if (!makefile_read) {
		...
		if (makefile_name->stat.is_file) {
			if (Makefile->stat.is_file && sun_style) {
				warning("Both `makefile' and `Makefile' exist");
			}
		...
		/* Just a kludge to avoid two slashes back to back */			
		   ...
		   if (slashp = strrchr(make_state->string_mb, '/')) {
			...
			tmp_path[slashp - make_state->string_mb]=0;
			  ...
			  warning("directory %s for .KEEP_STATE_FILE does not exist",tmp_path);
		        }
		        if (access(tmp_path, F_OK) != 0) {
			  warning("can't access dir %s",tmp_path);
		        }
		      }
		   }
		}
...
enter_argv_values(int argc, char *argv[], ASCII_Dyn_Array *makeflags_and_macro)
{
	...
	for (i = 1; i < argc; ++i) {
		...
		if (argv[i] == NULL) {
			...
			case 2:	/* -c seen */
				if (argv[i+1] == NULL) {
					fatal("No dmake rcfile argument after -c flag");
				}
				MBSTOWCS(wcs_buffer, NOCATGETS("DMAKE_RCFILE"));
			...
			case 4:	/* -g seen */
				if (argv[i+1] == NULL) {
					fatal("No dmake group argument after -g flag");
				}
				MBSTOWCS(wcs_buffer, NOCATGETS("DMAKE_GROUP"));
				name = GETNAME(wcs_buffer, FIND_LENGTH);
				break;
...
#endif
				if (argv[i+1] == NULL) {
					fatal("No dmake max jobs argument after -j flag");
				}
				MBSTOWCS(wcs_buffer, NOCATGETS("DMAKE_MAX_JOBS"));
				name = GETNAME(wcs_buffer, FIND_LENGTH);
				break;
			case 16: /* -M seen */
				if (argv[i+1] == NULL) {
					fatal("No pmake machinesfile argument after -M flag");
				}
				MBSTOWCS(wcs_buffer, NOCATGETS("PMAKE_MACHINESFILE"));
				name = GETNAME(wcs_buffer, FIND_LENGTH);
				break;
			case 32: /* -m seen */
				if (argv[i+1] == NULL) {
					fatal("No dmake mode argument after -m flag");
				}
				MBSTOWCS(wcs_buffer, NOCATGETS("DMAKE_MODE"));
				name = GETNAME(wcs_buffer, FIND_LENGTH);
				break;
			...
			case 256: /* -K seen */
				if (argv[i+1] == NULL) {
					fatal("No makestate filename argument after -K flag");
				}
			...
			case 512:	/* -o seen */
				if (argv[i+1] == NULL) {
					fatal("No dmake output dir argument after -o flag");
				}
				MBSTOWCS(wcs_buffer, NOCATGETS("DMAKE_ODIR"));
				name = GETNAME(wcs_buffer, FIND_LENGTH);
				break;
			case 1024: /* -x seen */
				if (argv[i+1] == NULL) {
					fatal("No argument after -x flag");
				}
				...
				if (strncmp(argv[i+1], NOCATGETS("DMAKE_OUTPUT_MODE="), length) == 0) {
					...
					warning("Unknown argument `%s' after -x flag (ignored)",
					      argv[i+1]);
			...
			if ((length = strlen(argv[i+1])) >= MAXPATHLEN) {
				...
				mbstowcs(tmp_wcs_buffer, argv[i+1], length + 1);
 ...
 */
			...
			*cp = tmp_char;
			...
			if ((length = strlen(cp)) >= MAXPATHLEN) {
				tmp_wcs_buffer = ALLOC_WC(length + 1);
				mbstowcs(tmp_wcs_buffer, cp, length + 1);
				value = GETNAME(tmp_wcs_buffer, FIND_LENGTH);
				retmem(tmp_wcs_buffer);
			} else {
...
append_makeflags_string(Name name, register String makeflags_string)
{
	...
	if (strcmp(name->string_mb, NOCATGETS("DMAKE_GROUP")) == 0) {
		...
		fatal("Internal error: name not recognized in append_makeflags_string()");
	}
...
read_environment(Boolean read_only)
{
	...
	for (; *environment; environment++) {
		...
		if ((length = strlen(*environment)) >= MAXPATHLEN) {
			...
			mbstowcs(tmp_wcs_buffer, *environment, length + 1);
			name = tmp_wcs_buffer;
		} else {
...
{
	fatal("can not get a license, exiting...");
	exit(1);
}
...
dmake_message_callback(char *err_msg)
{
	...
	if ((!list_all_targets) &&
	    (report_dependencies_level == 0) &&
	    (dmake_mode_type != serial_mode)) {
		warning("can not get a TeamWare license, defaulting to serial mode...");
	}
}
#endif
