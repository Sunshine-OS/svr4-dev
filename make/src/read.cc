...
/*	from OpenSolaris "read.cc	1.64	06/12/12"	*/
	...
	if ((makefile_name->hash.length != 1) ||
		...
		if ((makefile->body.makefile.contents == NULL) &&
			...
			if (makefile_path == NULL) {
				add_dir_to_path(".",
						&makefile_path,
						-1);
				add_dir_to_path(NOCATGETS(HDRSDIR),
						&makefile_path,
						-1);
#ifdef SUN5_0
				...
				add_dir_to_path(NOCATGETS("/etc/default"),
						&makefile_path,
						-1);
#elif defined(HP_UX) && defined (TEAMWARE_MAKE_CMN)
                		findrundir_err = avo_find_run_dir(&run_dir);
                		if (! findrundir_err) {
                        		sprintf(makerules_dir, NOCATGETS("%s/../share/lib/make"), run_dir);
		...
		if (lock_makefile && 
		    !do_not_exec_rule) {
			 ...
			 sprintf(make_state_lockfile,
						NOCATGETS("%s.lock"),
						make_state->string_mb);
			file_lock(make_state->string_mb,
					 make_state_lockfile,
...
enter_target_groups_and_dependencies(Name_vector target, Name_vector depes, Cmd_line command, Separator separator, Boolean target_group_seen)
{
	int			i;
	Boolean			reset= true;
	Chain			target_group_member = NULL;
	Percent			percent_ptr;
			...
			if (target->names[i] != NULL) {
				if (target_group_seen) {
					...
					if(target_group_member == NULL) {
						fatal_reader("Unexpected '+' on dependency line");
					}
				}
				reset = false;
